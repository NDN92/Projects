...
public class Converter {
	...
	public Object[] getJsExCodeForMultilineSetText(String[][] IdsWithStrings, AppFont font, int firstTextX, int firstTextY, double pageDpiRatio) {
		int lines = IdsWithStrings.length;
	...
	public Object[] getJsExcodeForInvoiceHeader(Object[][] objs, int labelWidth, AppFont font, int ihX1, int ihX2, int ihY, double pageDpiRatio) {
		...
		for( int i = 0; i < objs.length; i = i + 2 ) {
			String labelId 		= (String) objs[i][0];
			String labelName 	= (String) objs[i][1];
			String valueId 		= (String) objs[i+1][0];
			Object value 		= 		   objs[i+1][1];		
...
//			}
	...
	public int fontYOffset(AppFont font, int y) {		
		...
		return (int) Math.round(y + (font.getSize() - metrics.getDescent()) + Math.round( (font.getLineHeight() - font.getSize() ) / 2.0 ));
	}
